  ...
  try {
    ...
    try {
      ...
      while (attemptCount < maxAttempts) {
        ...
        console.log(`Calling Ideogram API (attempt ${attemptCount}/${maxAttempts})...`);
        
        // For retry attempts, modify the prompt to be safer
        let currentPrompt = prompt;
        let currentStyleType = style_type;
        
        if (isRetryAttempt) {
          console.log('Retrying with clean background prompt...');
          ...
          currentPrompt = prompt
            ...
            .replace(/Visual tags:.*$/i, '')
            .replace(/Tags:.*$/i, '')
            .trim();
          
          // Force DESIGN style for retry
        ...
        } else if (ideogramResponse.status === 422 && attemptCount < maxAttempts) {
          // Safety block error - try again with clean prompt
          const errorText = await ideogramResponse.text();
          console.error(`Ideogram safety block (attempt ${attemptCount}):`, errorText);
          console.log('Will retry with cleaned prompt...');
          continue;
